...
import com.wpisen.trace.agent.transfer.TransferTask;
import com.wpisen.trace.agent.transfer.UploadService;
import com.wpisen.trace.agent.transfer.UploadServiceImpl;
...
public class TraceContext implements AgentFinal {
	...
	public static final String DEBUG_CONSOLE_KEY = "trace_debug_";
	...
	public TraceContext(Properties properties) {
		...
		nodeQueue = new ArrayBlockingQueue<Object>(
				Integer.parseInt(properties.getProperty("trace.task.maxSize", "1000")));
		...
		serviceImpl.setHttpUrl(properties.getProperty("trace.upload.path", "http://log.wpisen.api/upload"));
		service = serviceImpl;
		try {
	...
	private void destorySession() {
		...
		if (System.getProperties().containsKey(Thread.currentThread())) {
			/*
			 * StackSession stackSession= (StackSession)
			 * System.getProperties().get(Thread.currentThread());
	...
	public void storeNode(Object traceNode) {
		if (!(traceNode instanceof TraceNode || traceNode instanceof TraceBeanWapper
				|| traceNode instanceof TraceFrom)) {
	...
	public int uploadNode(int size) {
		...
		if (!nodes.isEmpty() || !infos.isEmpty())
	...
	private boolean uploadWapperBean(TraceBeanWapper wapNode) {
		...
		if (upUrl.startsWith("http")) {
			service.uploadByHttp(wapNode.getNode() == null ? null : Arrays.asList(wapNode.getNode()),
					wapNode.getFrom() == null ? null : Arrays.asList(wapNode.getFrom()), upUrl);
			return true;
		}
		return false;
	}
