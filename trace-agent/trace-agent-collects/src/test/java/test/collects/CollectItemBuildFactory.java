...
public class CollectItemBuildFactory extends BasiceTest {
	...
	public void build() throws Exception {
		 ...
		 * objout.writeObject(list); String str = new
		 * sun.misc.BASE64Encoder().encode(output.toByteArray());
		 */
		/**
		 * 序列化成json 串
		 */
	...
	public List<AgentItemSource> createJdbcItems() throws NotFoundException, CannotCompileException, IOException {
		...
		String handClassName = "com.wpisen.trace.agent.collects.jdbc.JdbcDriverHandle";
	...
	public List<AgentItemSource> createDubboConsumerItems() throws Exception {
		List<AgentItemSource> list = new ArrayList<>();
		{
			...
			String handClassName = "com.wpisen.trace.agent.collects.dubbo.DubboConsumerRpcExceptionMonitorHandle";
			String method = "invoke";
			MethodInfo info = new MethodInfo(className, method);
			info.setReturnType(Object.class.getName());
			list.addAll(loadAgentItems(className, handClassName, "returnTemplate",
					"_param.put(\"invoker\",$0.invoker);", null, info));
		}
		{
			String className = "com.alibaba.dubbo.rpc.protocol.dubbo.filter.FutureFilter";
			String handClassName = "com.wpisen.trace.agent.collects.dubbo.DubboConsumerMonitorHandle";
			String method = "invoke";
			MethodInfo info = new MethodInfo(className, method);
	...
	public List<AgentItemSource> createDubboProviderItems() throws Exception {
		...
		String handClassName = "com.wpisen.trace.agent.collects.dubbo.DubboProviderMonitorHandle";
	...
	public List<AgentItemSource> createRedisItems() throws Exception {
		...
		String handClassName = "com.wpisen.trace.agent.collects.redis.RedisConnectionHandle";
		MethodInfo info = new MethodInfo();
		info.setClassName(className);
		info.setReturnType("redis.clients.jedis.Connection");
		info.setMethodName("sendCommand");
		info.setParamTypes(new String[] { "redis.clients.jedis.Protocol$Command", "[[B" });
	...
	public void agentXmlFormatTest() throws Exception {
		...
		AgentPlugin agentPlugin = JaxbUtil.converyToJavaBean(new String(bytes, "UTF-8"), AgentPlugin.class);
